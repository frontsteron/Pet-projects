...
<!--  Или пишем в теге head, но с атрибутом defer, тогда скрипт выполнится когда будут загружены/созданы все теги(более современный вариант), а к в внешний URL js файла подключаем с атрибутом integrity для безопасности/для проверки контрольной суммы -->
```
### А подключаем скрипты в **html** документ через тег **scrypt**.
---
### Имена переменных и функций в **JavaScript** начинаются с буквы, $ или символа подчеркивания, могут даже содержать эмодзи или иероглифы, и регистр букв имеет значение. А вот зарезервированные слова, которые нельзя использовать в названиях имен:
```JavaScript
...
## В JS есть следующие типы данных(*данные пишем в camelCase*):
...
### Существует **let** и **const** для хранения данных. До пременной **let** использовали **var**, но сейчас она является устаревшей, вместо неё используем **let**. А по умолчанию нужно использовать **const**, её обновить не можем, она хранит иммутабельные(*immutability*) переменные. При необходимости обновить значение переменной **let**, обновляем её через простое равно(*оператор assign*(*=*)).
...
age = 40
```
### *PS.Обратим внимание, что в отличие от некоторых других языков программирования, в **JavaScript** нет отдельного типа данных для символов, и символы представлены типом данных **symbol**. Кроме того, в **JavaScript** есть тип данных **bigint**, который позволяет работать с числами произвольной длины, но он является экспериментальным и поддерживается не во всех браузерах. А ещё в **JavaScript** примитивные типы данных передаются по значению, а не по ссылке.*
---
## Функции:
