...
<!--  Или пишем в теге head, но с атрибутом defer, тогда скрипт выполнится когда будут загружены/созданы все теги(более современный вариант) -->
```
---
...
### Существует **let** и **const** для хранения данных. *PS. До пременной **let** использовали **var**, но сейчас она является устаревшей, вместо неё используем **let***. А по умолчанию нужно использовать **const**, её обновить не можем. При необходимости обновить значение переменной **let**, обновляем её через простое равно(*оператор assign*(*=*)).
