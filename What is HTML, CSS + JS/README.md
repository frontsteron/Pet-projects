...
* **null** - тип данных, который имеет только одно значение - null. Он используется для указания на то, что значение переменной отсутствует.
```JavaScript
const password = null;
```

* **undefined** - тип данных, который имеет только одно значение - undefined. Он используется для указания на то, что переменная не имеет значения.
```JavaScript
let name;
...
age = 40
```
### Обратите внимание, что в отличие от некоторых других языков программирования, в **JavaScript** нет отдельного типа данных для символов, и символы представлены типом данных **symbol**. Кроме того, в **JavaScript** есть тип данных **bigint**, который позволяет работать с числами произвольной длины, но он является экспериментальным и не поддерживается во всех браузерах. Обратите внимание, что в **JavaScript** примитивные типы данных передаются по значению, а не по ссылке.

### Так же он предоставляет широкий спектр встроенных функций, которые можно использовать для различных целей, например:
...
### Обратите внимание, что это лишь небольшая подборка из множества функций, доступных в JavaScript. Существует множество других функций и библиотек, которые можно использовать для самых разных целей, от манипулирования строками и массивами до выполнения сложных вычислений или взаимодействия с веб-интерфейсами.

## Условные выражения
* **if** - используется для выполнения блока кода, если выражение, переданное в скобках, истинно.
* **else** - используется для выполнения блока кода, если выражение, переданное в скобках, ложно.
* **else if** - используется для выполнения блока кода, если первое выражение ложно, но следующее выражение истинно.
```JavaScript
if (condition1) {
  // код, который нужно выполнить, если выражение1 истинно
} else if (condition2) {
  // код, который нужно выполнить, если выражение2 истинно
} else {
  // код, который нужно выполнить, если оба выражения ложны
}
```
* **switch** - используется для выполнения блоков кода в зависимости от значения выражения.
```JavaScript
switch (expression) {
  case value1:
    // код, который нужно выполнить, если expression равно value1
    break;
  case value2:
    // код, который нужно выполнить, если expression равно value2
    break;
  default:
    // код, который нужно выполнить, если expression не равно ни одному значению
}
```
## Циклы JS
