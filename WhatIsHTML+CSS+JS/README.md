...
<!--  Или пишем в теге head, но с атрибутом defer, тогда скрипт выполнится когда будут загружены/созданы все теги(это более современный вариант, а в документе XHTML пишем атрубут defer="defer"). Атрибут async говорить, что можно продолжить загрузку страницы, не дожидаясь завершения загрузки сценария(в документе XHTML пишем атрубут async="async"). К в внешний URL js файла подключаем с атрибутом integrity для безопасности/для проверки контрольной суммы -->
```
...
---
## Синтаксис:
* ### Имена переменных(*идентификатор*) и функций в **JS** начинаются с **буквы**, **$**, или с символа подчеркивания(_), *могут даже содержать эмодзи или иероглифы*.
* ### **JS** чувствителен к регистру,
* ### Используем нотацию **camelCase**,
...
```
## В JS храним данные с помощью следующих ключевых слов:
